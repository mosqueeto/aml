...
static unsigned int save_low_core[256];
static unsigned int *chkp;
...
{
#ifdef STANDALONE
	fprintf( stderr, "%s: ", pgmName );	/* Hal 89.02.25 */
...
void leave(char *s)
{
	...
	if(trace) {
		chkp = NULL;
		for( i=0;i<256;i++) {
			if( *chkp++ != save_low_core[i] ) {
				printf("Low core trashed at %d\n",i);
				exit(1);
			}
		}
		traceinit();
...
void leaveint(char *s ,int n)
{
	...
	if(trace) {
		chkp = NULL;
		for( i=0;i<256;i++) if( *chkp++ != save_low_core[i] ) {
			printf("Low core trashed at %d\n",i);
			exit(1);
		}
		traceinit();
