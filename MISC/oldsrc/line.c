 ...
 *	It isn't here for any good reason.  Note that this code only
 *	updates the dot and mark values in the window list.  Since
 *	all the code acts on the current window, the buffer that we
 *	are editing must be being displayed, which means that
 *	"b_nwnd" is non zero, which means that the dot and mark
 *	values in the buffer headers are nonsense. 
 *
 *History:	
 */
