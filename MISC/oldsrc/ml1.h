/*
 *	file: ml1.h
 *		-- header file for the ml1 compiler
 *
 *	Copyright (c) 1989 by Kent Crispin and Songbird Software.
 *	All rights reserved.
 *
 *Modification History:
 *89.02.12 Hal -- added new data structures for implementing songs as
 *		a data abstraction.
 *89.01.13 Hal -- added ';' to end of struct definitions for MPW compiler
 *		also can't have floating in int initializer
 */

#ifdef ML1H_DEFD
#else
#define ML1H_DEFD 1

#include <ctype.h>
#include <stdio.h>
#include <setjmp.h>

#define TRUE 1
#define FALSE 0

#define COMPILE_ERR -1

#define EMPHASIZE (1.1)
#define DEEMPHASIZE (0.9)
#define TUNING_BIAS (-52)		/* bias note numbers for a DX7 */
#define DEFAULT_TEMPO 60

/* event codes */
#define NOTE_ON_EVENT		1
#define NOTE_OFF_EVENT		2
#define REST_ON_EVENT		3
#define REST_OFF_EVENT		4

/* key assignments */
#define OCTAVE_UP	'/'
#define OCTAVE_DOWN	'\\'
#define SHARP		'+'
#define FLAT		'-'
#define NATURAL		'='
#define BEGIN_SEQ	'['
#define END_SEQ		']'
#define BEGIN_CHORD	'{'
#define END_CHORD	'}'
#define ACCENT		'!'
#define SUBDUE		'?'
#define TIE		'_'
#define BEGIN_PARAM	'-'
#define BEGIN_FUNC	'('
#define END_FUNC	')'

/*
 * 	misc constants
 */
#define EOI 		-1
#define DID_NOTES	1
#define NO_NOTES	2

/*
 *	Useful Macros Hal 89.02.12
 */
#define	MIN(a,b)	((a)<(b)?(a):(b))
#define	MAX(a,b)	((a)>(b)?(a):(b))
#define	BOOLEAN(a)	((a)?true:false)
#define	LXOR(a,b)	(BOOLEAN(a) ^ BOOLEAN(b))

/*
 *	Useful types
 */
typedef enum {false,true} boolean;	/* Hal 89.02.12 */

typedef unsigned char byte;	/* basically a small unsigned integer */

struct node_struct {
	struct node_struct *next;
	float	delay;
	float	volume;
	byte	event;
	byte	note;
	byte	channel;
};
typedef struct node_struct node;

struct scope_env {
	float	duration;
	float	volume;
	byte	tempo;
	byte	velocity;
	byte	channel;
	char	key[7];	/* really signed */
	byte	octave;
	byte	articulation;
	char	transpose;
};
typedef struct scope_env ENVIRONMENT;

#ifdef NEWDS
/*
 *	An ML song is compiled into a series of events, which
 *	bear a strong resemblance to MIDI commands.  Here is this
 *	basic structure. (Hal 89.02.06)
 */
typedef struct {
	byte 	event;		/* ML1, not MIDI, event code */
	byte	note;
	byte	vel;
	byte	chan;
	int 	delay;
} SongEvent;

/*
 *	The play sequence of events is subject to many variables.  In
 *	particular, it may be that only a portion of an entire song
 *	is to be played.
 *
 *	Also, neither the compiler output nor the play routine really
 *	need to know _how_ the sequence is maintained, hence
 *	the routines get and put SongEvents indirectly via the following
 *	structure.
 *
 *	The code which implements the structure will also provide
 *	routines to allow the creation and destruction of sequences.
 *	(Hal 89.02.06) 
 */
typedef struct SongEventSequence {
	SongEvent *(*getEvent)( struct SongEventSequence * );
	boolean	(*putEvent)( struct SongEventSequence *,
			int event, int note, int velocity,
			int channel, int delay );
} SongEventSequence;
#endif

#ifdef MAIN
/*
 *	Set up the default environment
 */
ENVIRONMENT song_env =
	{ {1.0},		/* duration */
	  {1.0}, 		/* volume */
	  {0},			/* tempo */
	  {0},			/* velocity */
	  {0},			/* channel */
	  {0,0,0,0,0,0,0}, 	/* key signature */
	  {4},			/* octave */
	  {64},			/* articulation */
	  {64},			/* transpose */
	};
jmp_buf state_buf;
char *	pgmName = NULL;	/* name of this program */
int trace	= 0; 
FILE *tfile	= NULL;
int debugflag	= 0;
unsigned int *nullp = NULL;	/* for debugging purposes */
int nnodes	= 0;
/*
 *	Global state determining default duration of a beat, default
 *	volume parameters, and so on
 */
 
int tempo = DEFAULT_TEMPO;	/* tempo in beats per minute.  This */
				/* is the fundamental time keeper. */
				
byte beat_duration_value = 16;	/* quarter note gets beat, by default */
				
int msec_per_beat =  (1000*60) / DEFAULT_TEMPO;	/* Hal 89.01.13 */
				/* msec_per_beat is milliseconds per */
				/* beat -- a function of the tempo.  */
				
int song_volume	= 64;		/* this is the default value that is  */
				/* converted to a midi velocity figure.*/
				/*  It is multiplied by the entity_volume*/
				/*  and relative_volume to get the final*/
				/*  note velocity. It ranges from 0 to 127*/

#ifndef NEWDS
byte 	*event	= NULL;		/* parallel arrays that store a compiled*/
byte	*note	= NULL;		/* song before it is played */
byte	*vel	= NULL;
byte	*chan	= NULL;
int 	*delay	= NULL;
int 	output_index = 0;
#endif

#else
extern jmp_buf state_buf;
extern	char *	pgmName;	/* name of this program */
extern int trace;
extern FILE *tfile;
extern int debugflag;
extern unsigned int *nullp;
extern int nnodes;
extern int tempo;
extern byte beat_duration_value;
extern int msec_per_beat;
extern int song_volume;
#ifndef NEWDS
extern byte *event,*note,*vel,*chan;
extern int *dlay;
extern int output_index;
#endif
#endif


float	do_accent(char c);
node    *do_chord( ENVIRONMENT env );
int	do_entity( ENVIRONMENT *env);
void	do_function( ENVIRONMENT *env);
float	do_named_duration(char named_duration);
node	*do_note( char base_note_name, ENVIRONMENT *env);
void	do_parameter( ENVIRONMENT *env );
int	do_relative_duration(char c);
node	*do_sequence( ENVIRONMENT env );
void	do_song( ENVIRONMENT env );
void	enter( char *s );
void	error(char *s1, void *s2);	/* Hal 89.02.25 */
void	free_node(node *n);
int	get_num();
void	init_io(char *ifname,char *ofname);
int	is_named_duration( char c );
int	is_note_name( char c );
void	leave( char *s );
void	leavechar( char *s, char c );
void	leavefloat( char *s1, float f);
void	leaveint( char *s, int i );
void	leavestring( char *s1, char *s2 );
node	*new_node();
int	nextc();
void	output( node *list );
void	parse_error(char *s);
#ifdef NEWDS
void play( SongEventSequence * theSequence, int repeats );
#else
int	play(int repeats);
#endif
int	pushc(char c);

#endif
