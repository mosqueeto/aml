...
#else
	/* otherwise we use the editor's buffer */
	cur_line = curwp->w_dotp;
	cur_ofst = curwp->w_doto;
#endif
...
int ml1_getc()
{
	...
	if( cur_line == NULL ) 
		return COMPILE_ERR;
	else if( cur_ofst == llength(cur_line) ) {
		c = '\n';
		cur_line = lforw(cur_line);
		cur_ofst = 0;
		if( cur_line == curbp->b_linep ) c = EOI;
	} else {
		c = lgetc(cur_line,cur_ofst);
		cur_ofst++;
	}
	nchars_parsed++;
	return c;
...
#else
	if( push_top > 16 ) {
		error("Too many characters pushed",NULL);
		return COMPILE_ERR;
	}
	push_stk[ push_top++ ] = c;		
	nchars_parsed--;
...
void parse_error(char *s)
{
	...
	printf("%s\n",s);
#else
	forwchar(TRUE,nchars_parsed-1);
	mlwrite("Parse error: %s",s);
	longjmp(state_buf,-1);
#endif
}
...
void output( node *n )
{
	...
	while( n != NULL ) {
		...
		if( n->type == A_NOTE ) {
			...
			n->type      = NOTE_ON;
			np = new_node();
			np->start    = n->start + n->duration * (n->duty/100.0);
			np->duration = 0.0;
			np->volume   = 0.0;
			np->type     = NOTE_OFF;
			np->duty     = n->duty;
